...
class ELLIPSE:
    def __init__(
        ...
        self.time = time #simulation time [start,end]
    ...
    def generate_domain(self, N, t=None):
        T0,T1,T2 = self.time[0],self.time[1],self.time[2]
    ...
    def generate_boundary(self, grid=[20,10,5]):
        ...
        boundary_t = np.tile(np.expand_dims(np.linspace(T0, T1, Nt), axis=(1,2)),(1,Nl,1))
